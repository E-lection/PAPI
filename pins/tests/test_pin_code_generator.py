...
class PinCodeGeneratorTests(TestCase):
    ...
    def test_ineligible_voter_pin_generation_returns_none(self, get_and_check_votability):
    ...
    def test_eligble_voter_pin_generation_returns_pin(self, SystemRandom, get_and_check_votability):
        self.assertEqual(generate_pin_code(STATION_ID, ELIGIBLE_VOTER_ID), 123456)

    @patch('pins.pin_code_generator.SystemRandom.randrange', return_value=123456)
    @patch('pins.pin_code_generator.get_and_check_votability', return_value=True)
    def test_existing_pin_for_eligible_voter_is_replaced(self, SystemRandom, get_and_check_votability):
    ...
    @patch('pins.pin_code_generator.get_and_check_votability', return_value=True)
    def test_pins_are_unique_to_station(self, SystemRandom, get_and_check_votability):
