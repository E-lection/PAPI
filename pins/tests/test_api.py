...
class VerifyPinCodeTests(TestCase):
    ...
    @patch('pins.views.has_verify_and_check_eligibility_permissions', return_value=True)
    def test_endpoint_returns_repsonse(self, *_):
        url = reverse('pins:verify_pin_code_and_check_eligibility',
                      args=(STATION_PK, VALID_PIN,))
        response = self.client.get(url, **{'HTTP_AUTHORIZATION': '123'})

        self.assertEqual(response.status_code, RESPONSE_OK)

    @patch('pins.views.has_verify_and_check_eligibility_permissions', return_value=True)
    def test_verify_pin_for_invalid_pin(self, *_):
        url = reverse('pins:verify_pin_code_and_check_eligibility',
                      args=(STATION_PK, INVALID_PIN,))
        response = self.client.get(url, **{'HTTP_AUTHORIZATION': '123'})
